...
func int B_CheckItem(var int category,var int slot)
{
	...
	if(count > 0)
	{
		...
		if(self.aivar[36] && (Npc_GetPermAttitude(self,other) == ATT_ANGRY) && b_hasplayersilvertosatisfy() && Hlp_IsItem(item,itmi_silver))
		{
			PrintDebugNpc(PD_ZS_Check,"B_CheckItem ... genug Silber nach Diebstahl");
			b_exchangememoryattitude();
			return TRUE;
		};
		if(self.aivar[AIV_BEENATTACKED] && (Npc_GetPermAttitude(self,other) == ATT_ANGRY) && b_hasplayersilvertosatisfy() && Hlp_IsItem(item,itmi_silver))
		{
			PrintDebugNpc(PD_ZS_Check,"B_CheckItem ... genug Silber nach Diebstahl");
			b_exchangememoryattitude();
			return TRUE;
